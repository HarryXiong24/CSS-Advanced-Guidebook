...
font-size属性表示字体大小，这个属性一般用的单位为px。也可以用一般表示大小的单词来直接表示大小。

这个大小是由浏览器加上的，如:xx-small x-small等绝对尺寸，smaller和larger等相对尺寸以及百分比尺寸。
...
- normal 表示正常字体
- italic 和 oblique 都是表示斜体,但是实质上还是有差别，一般都用italic

### 字体显示小型大写字符font-variant
